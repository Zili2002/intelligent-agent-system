...
- `daemon` 支持运行间隔、最大持续时间、最大循环次数和优雅停止。
- 每个 Mission 使用跨进程锁，能够回收无主的陈旧锁。
- 网络和运行时瞬时错误使用有上限的指数退避重试。
- 中断的 approved/running 实验会优先恢复，不重复设计新实验。
- `runs`、`history`、`health` 提供结构化运行记录和健康状态。
- 实验与付费 LLM 请求统一进入可审计审批队列。
